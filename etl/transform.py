...
def ensure_columns(df: pd.DataFrame, expected_cols: List[str]) -> pd.DataFrame:
    """Ensure DataFrame has all expected columns; fill missing with None and order columns."""
...
def transform_teams(teams_data: List[Dict]) -> pd.DataFrame:
    ...
    return ensure_columns(df, expected_cols)

def transform_gameweeks(events_data: List[Dict]) -> pd.DataFrame:
    """Minimal transformation for gameweeks; expand chip plays."""
...
def transform_player_positions(element_types_data: List[Dict]) -> pd.DataFrame:
    """Minimal transformation for player positions."""
...
def transform_players(players_data: List[Dict], teams_map: Dict) -> pd.DataFrame:
    """Minimal transformation for players: map team, scale cost, select columns."""
    df = pd.DataFrame(players_data)
    cols = ['id', 'web_name', 'team', 'element_type', 'now_cost', 'total_points',
...
def transform_fixtures(fixtures_data: List[Dict]) -> pd.DataFrame:
    """Minimal transformation for fixtures; ensure all columns present, serialize stats."""
    expected_cols = [
...
def transform_player_history(player_id: int, web_name: str, history_data: Dict) -> pd.DataFrame:
    """Transform current season player history into a DataFrame."""
